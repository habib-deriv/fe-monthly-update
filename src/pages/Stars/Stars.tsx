...
import { DeleteOutline, Close, Edit, DoneAll, FileDownload } from '@mui/icons-material';
...
const Stars = () => {
    ...
    const [is_loading, items, grouped_items] = useFetch(COLLECTIONS.STARS, 'month', settings.list.months);

    return (
        <div className="p-10">
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center justify-start basis-1/3">
                    <h3 className="uppercase text-lg font-bold mr-2">Stars of the month</h3>
                    <Chip
            ...
            <div>
                ...
                {!is_loading && has_items_in_month(items, settings.date.month) && items && items.length && grouped_items[settings.date.month].map((item) => {
                    return (
                        <div key={item.id}
                        ...
                        >
                            <div className="flex items-center justify-start">
                                ...
                                <div className="ml-4">
                                    ...
                                    <ul>
                                        {item.achievements.split(' | ').map((achievement: string, idx: number) => {
                                            return (
                                                <li
                                                    key={idx}
